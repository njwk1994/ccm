...
public class FlowTaskServiceImpl extends FlowServiceFactory implements IFlowTaskService {
     ...
     */
    @Override
    public Result applyTask(FlowTaskVo flowTaskVo) {
        if (StringUtils.isNotBlank(flowTaskVo.getDataId())) {
            try {
                QueryWrapper<FlowMyBusiness> flowMyBusinessQueryWrapper = new QueryWrapper<>();
                flowMyBusinessQueryWrapper.eq("data_id", flowTaskVo.getDataId());
                FlowMyBusiness business = flowMyBusinessService.getOne(flowMyBusinessQueryWrapper);
                if (business != null && StringUtils.isNotBlank(business.getId())) {
                    if (StringUtils.isNotBlank(business.getProcessInstanceId()) && StringUtils.isNotBlank(business.getTaskId())) {
                        ...
                        flowMyBusinessService.deleteFlowMyBusiness(business.getId());
                        return this.firstStartProcess(flowTaskVo);
                    }
                } else {
                    return this.firstStartProcess(flowTaskVo);
                }
            ...
            return Result.error("请先选择一条实体数据！");
        }
     ...
     * 根据实体数据id发起流程
     */
    private Result firstStartProcess(FlowTaskVo flowTaskVo) {
        FlowMyBusiness business = new FlowMyBusiness();
        business.setDataId(flowTaskVo.getDataId());
        business.setTitle(flowTaskVo.getTitle());
        if (StringUtils.isNotBlank(flowTaskVo.getProcessDefKey())) {
            business.setProcessDefinitionKey(flowTaskVo.getProcessDefKey());
            ProcessDefinition processDefinition = repositoryService.createProcessDefinitionQuery().processDefinitionKey(flowTaskVo.getProcessDefKey()).latestVersion().singleResult();
            ...
            if (flowTaskVo.getVariables() != null && flowTaskVo.getVariables().size() > 0) {
                variables = flowTaskVo.getVariables();
            }
            variables.put(ProcessConstants.PROCESS_INITIATOR, sysUser.getUsername());
            // 添加对象UID到全局变量
            variables.put("objectUid", flowTaskVo.getDataId());
            ...
            if (Objects.nonNull(firstTask)) {
                ...
                if (StringUtils.isBlank(comment)) {
                    comment = sysUser.getRealname() + "：发起申请！";
                }
                taskService.addComment(firstTask.getId(), processInstance.getProcessInstanceId(), FlowComment.NORMAL.getType(), comment);
                taskService.setAssignee(firstTask.getId(), sysUser.getUsername());
            }
            /*======================todo 启动之后  回调以及关键数据保存======================*/
            //设置数据
            FlowNextDto nextFlowNode = this.getNextFlowNode(firstTask.getId(), variables);
            taskService.complete(firstTask.getId(), variables);
            //下一个实例节点  多实例会是一个list,随意取一个即可  数组中定义Key是一致的
            List<Task> todoTaskList = taskService.createTaskQuery().processInstanceId(processInstance.getProcessInstanceId()).active().list();
            Task presentTask = null;
            if (todoTaskList.size() > 0) presentTask = todoTaskList.get(0);
            JSONArray doneUserList = new JSONArray();
            doneUserList.add(sysUser.getUsername());
            //spring容器类名
            String serviceImplName = "myFlowService";
            business.setServiceImplName(serviceImplName);
            FlowCallBackServiceI flowCallBackService = (FlowCallBackServiceI) SpringContextUtils.getBean(serviceImplName);//spring容器类名 自定义业务
            if (nextFlowNode != null) {
                //**有下一个节点
                UserTask nextTask = nextFlowNode.getUserTask();
                //能够处理下个节点的候选人
                List<String> collect_username = nextFlowNode.getUserList().stream().map(SysUser::getUsername).collect(Collectors.toList());
                List<String> beforeParamsCandidateUsernames = flowCallBackService.flowCandidateUsernamesOfTask(presentTask.getTaskDefinitionKey(), variables);//业务层指定候选人
                if (CollUtil.isNotEmpty(beforeParamsCandidateUsernames)) {
                    // 删除后重写
                    for (Task taskItem : todoTaskList) {
                        for (String oldUser : collect_username) {
                            taskService.deleteCandidateUser(taskItem.getId(), oldUser);
                        }
                    }
                    // 业务层有指定候选人，覆盖
                    for (Task taskItem : todoTaskList) {
                        for (String newUser : beforeParamsCandidateUsernames) {
                            taskService.addCandidateUser(taskItem.getId(), newUser);
                        }
                    }
                    business.setTodoUsers(JSON.toJSONString(beforeParamsCandidateUsernames));
                }
                business.setProcessDefinitionId(processDefinition.getId());
                business.setProcessInstanceId(processInstance.getProcessInstanceId());
                business.setActStatus(ActStatus.start);
                business.setProposer(sysUser.getUsername());
                business.setTaskId(presentTask.getId());
                business.setTaskName(nextTask.getName());
                business.setTaskNameId(nextTask.getId());
                business.setPriority(nextTask.getPriority());
            ...
            flowMyBusinessService.save(business);
            business.setVariables(variables);
            if (flowCallBackService != null) {
                flowCallBackService.afterFlowHandle(business);// 流程处理完后，进行回调业务层
            }
            return Result.OK("流程启动、发起申请 成功！");
        } else {
            return Result.error("流程定义Key有错！");
        }
    }
    ...
    public Result complete(FlowTaskVo taskVo) {
        Task presentTask = taskService.createTaskQuery().taskId(taskVo.getTaskId()).singleResult();
        if (Objects.isNull(presentTask)) {
            return Result.error("任务不存在");
        }
        SysUser loginUser = iFlowThirdService.getLoginUser();
        if (DelegationState.PENDING.equals(presentTask.getDelegationState())) {//委托、委派  PENDING待解决
        ...
        // 前后端流程变量 参与业务逻辑
        Map<String, Object> variableValue = taskVo.getVariables();
        // 2022.10.18 HT 通过的任务添加approved属性
        variableValue.put("approved", true);
        Map<String, Object> flowBeforeParamsValues = flowCallBackService.flowValuesOfTask(business.getTaskNameId(), variableValue);
        if (MapUtil.isNotEmpty(flowBeforeParamsValues)) {
            for (Map.Entry<String, Object> entry : flowBeforeParamsValues.entrySet()) {
                if (!variableValue.containsKey(entry.getKey())) {
                    variableValue.put(entry.getKey(), entry.getValue());
                }
            }
        }
        FlowNextDto nextFlowNode = this.getNextFlowNode(presentTask.getId(), variableValue);//根据条件获取下一个实际节点
        //结束当前节点
        if (DelegationState.PENDING.equals(presentTask.getDelegationState())) {
            taskService.resolveTask(taskVo.getTaskId(), variableValue);
        } else {
            taskService.complete(taskVo.getTaskId(), variableValue);
        }
        //下一个实例节点
        List<Task> todoTaskList = taskService.createTaskQuery().processInstanceId(business.getProcessInstanceId()).active().list();
        Task todoTask = null;
        if (CollUtil.isNotEmpty(todoTaskList)) {
            todoTask = todoTaskList.get(0);
        }
        // 下个节点候选人以前后端指定为主
        List<String> candidateUsers = taskVo.getCandidateUsers();//前端传入候选人
        if (CollUtil.isEmpty(candidateUsers)) {
            if (todoTask != null) {
                candidateUsers = flowCallBackService.flowCandidateUsernamesOfTask(todoTask.getTaskDefinitionKey(), variableValue);//业务层指定候选人
            }
        }
        // 处理过流程的人
        ...
        if (todoTask != null && presentTask.getTaskDefinitionKey().equals(todoTask.getTaskDefinitionKey())) {
            // * 当前节点是会签节点，没有走完
            business.setActStatus(ActStatus.doing).setTaskId(todoTask.getId()).setDoneUsers(doneUserList.toJSONString());
            //以下全是在处理候选处理人
            JSONArray todosArr = JSON.parseArray(business.getTodoUsers());
            // 删除后重写
            for (Task taskItem : todoTaskList) {
                for (Object oldUser : todosArr) {
                    taskService.deleteCandidateUser(taskItem.getId(), oldUser.toString());
                }
            }
            // 重写
            if (CollUtil.isNotEmpty(candidateUsers)) {
                candidateUsers.remove(loginUser.getUsername());
                for (Task taskItem : todoTaskList) {
                    for (String newUser : candidateUsers) {
                        taskService.addCandidateUser(taskItem.getId(), newUser);
                    }
                }
                business.setTodoUsers(JSON.toJSONString(candidateUsers));
            } else {//前端业务层都没指定
                todosArr.remove(loginUser.getUsername());
                for (Task taskItem : todoTaskList) {
                    for (Object oldUser : todosArr) {
                        taskService.addCandidateUser(taskItem.getId(), oldUser.toString());
                    }
                }
            ...
            if (nextFlowNode != null) {
                UserTask nextTask = nextFlowNode.getUserTask();//**有下一个节点
                List<String> collect_username = new ArrayList<>();
                if (CollUtil.isNotEmpty(candidateUsers)) {//端传或后台指定候选人
                    collect_username = candidateUsers;
                } else {
                    collect_username = nextFlowNode.getUserList().stream().map(SysUser::getUsername).collect(Collectors.toList());
                }
                business.setActStatus(ActStatus.doing).setTaskId(todoTask.getId()).setTaskNameId(nextTask.getId()).setTaskName(nextTask.getName()).setPriority(nextTask.getPriority()).setDoneUsers(doneUserList.toJSONString()).setTodoUsers(JSON.toJSONString(collect_username));
                // 删除后重写 以下也是在处理候选人，不重要
                for (Task taskItem : todoTaskList) {
                    for (String oldUser : collect_username) {
                        taskService.deleteCandidateUser(taskItem.getId(), oldUser);
                    }
                }
                if (CollUtil.isNotEmpty(candidateUsers)) {// 前端没有传入候选人 && 业务层有指定候选人，覆盖
                    for (Task taskItem : todoTaskList) {
                        for (String newUser : candidateUsers) {
                            taskService.addCandidateUser(taskItem.getId(), newUser);
                        }
                    ...
                    for (Task taskItem : todoTaskList) {
                        for (String oldUser : collect_username) {
                            taskService.addCandidateUser(taskItem.getId(), oldUser);
                        }
                    }
                    business.setTodoUsers(JSON.toJSONString(collect_username));
                }
            } else {//**没有下一个节点，流程已经结束了
                business.setActStatus(ActStatus.pass).setDoneUsers(doneUserList.toJSONString()).setTodoUsers("").setTaskId("").setTaskNameId("").setTaskName("");
            }
        }
        flowMyBusinessService.updateById(business);
        if (flowCallBackService != null) {
            // 流程处理完后，进行回调业务层
            business.setVariables(variableValue);
    ...
    public void taskReject(FlowTaskVo flowTaskVo) {
        ...
        try {
            ...
            FlowCallBackServiceI flowCallBackService = (FlowCallBackServiceI) SpringContextUtils.getBean(serviceImplName);
            //前端流程变量值
            Map<String, Object> variableValue = flowTaskVo.getVariables();
            // 2022.10.18 HT 拒绝的任务添加!approved属性
            variableValue.put("approved", false);
            List<String> beforeParamsCandidateUsernames = flowCallBackService.flowCandidateUsernamesOfTask(task2.getTaskDefinitionKey(), variableValue);
            //设置数据
            String doneUsers = business.getDoneUsers();
            // 处理过流程的人
            JSONArray doneUserList = new JSONArray();
            ...
            business.setActStatus(ActStatus.reject).setTaskId(task2.getId()).setTaskNameId(task2.getTaskDefinitionKey()).setTaskName(task2.getName()).setDoneUsers(doneUserList.toJSONString());
    ...
    public void taskReturn(FlowTaskVo flowTaskVo) {
        ...
        try {
            ...
            business.setActStatus(ActStatus.reject).setTaskId(targetTask.getId()).setTaskNameId(targetTask.getTaskDefinitionKey()).setTaskName(targetTask.getName()).setPriority(targetTask.getPriority() + "").setDoneUsers(doneUserList.toJSONString());
            if (target != null) {
                ...
                if (StrUtil.equals(business.getTaskNameId(), ProcessConstants.START_NODE)) {
                    ...
                    Map<String, Object> values = flowTaskVo.getVariables();
                    if (values == null) {
            ...
            flowMyBusinessService.updateById(business);
            // 流程处理完后，进行回调业务层
            business.setVariables(flowTaskVo.getVariables());
    ...
    public Result myProcess(Integer pageNum, Integer pageSize) {
        ...
        HistoricProcessInstanceQuery historicProcessInstanceQuery = historyService.createHistoricProcessInstanceQuery().startedBy(username).orderByProcessInstanceStartTime().desc();
    ...
    public Result stopProcess(FlowTaskVo flowTaskVo) {
        ...
        if (Objects.nonNull(bpmnModel)) {
            ...
            if (CollectionUtils.isNotEmpty(endNodes)) {
                ...
                taskService.addComment(task.get(0).getId(), processInstance.getProcessInstanceId(), FlowComment.STOP.getType(), StringUtils.isBlank(flowTaskVo.getComment()) ? "取消申请" : flowTaskVo.getComment());
    ...
    public Result todoList(Integer pageNum, Integer pageSize) {
        Page<FlowTaskDto> page = new Page<>();
        String username = iFlowThirdService.getLoginUser().getUsername();
        TaskQuery taskQuery = taskService.createTaskQuery().active().includeProcessVariables().taskAssignee(username).orderByTaskCreateTime().desc();
    ...
    public Result finishedList(Integer pageNum, Integer pageSize) {
        Page<FlowTaskDto> page = new Page<>();
        String username = iFlowThirdService.getLoginUser().getUsername();
        HistoricTaskInstanceQuery taskInstanceQuery = historyService.createHistoricTaskInstanceQuery().includeProcessVariables().finished().taskAssignee(username).orderByHistoricTaskInstanceEndTime().desc();
                    ...
                    for (HistoricIdentityLink identityLink : linksForTask) {
                        if (IdentityLinkType.CANDIDATE.equals(identityLink.getType())) {
                            if (StringUtils.isNotBlank(identityLink.getUserId())) {
                                try {
                                    SysUser sysUser = iFlowThirdService.getUserByUsername(identityLink.getUserId());//此处还需深入研究
                                    stringBuilder.append(sysUser.getRealname()).append(",");
                                } catch (Exception ex) {
                                    stringBuilder.append("管理员").append(",");//这个地方还要追查为什么会出现jeecg
                                }
                            }
    ...
    public InputStream diagram(String processId) {
        ...
        InputStream in = diagramGenerator.generateDiagram(bpmnModel, "png", highLightedNodes, highLightedFlows, configuration.getActivityFontName(), configuration.getLabelFontName(), configuration.getAnnotationFontName(), configuration.getClassLoader(), 1.0, true);
        return in;
    }
    ...
    public List<SysUser> getSysUserFromTask(UserTask userTask) {
        ...
        if (CollUtil.isNotEmpty(candidateGroups)) {
            ...
            for (String candidateGroup : candidateGroups) {
                if (candidateGroup.startsWith("G*_")) {
                    List<SysUser> usersByRoleId = iFlowThirdService.getUsersByRoleId(candidateGroup);
                    userList.addAll(usersByRoleId);
                } else if (candidateGroup.startsWith("O*_")) {
                    List<SysUser> usersByRoleId = iFlowThirdService.getUsersByOrganizationId(candidateGroup);
                    userList.addAll(usersByRoleId);
                } else {
                    List<SysUser> usersByRoleId = iFlowThirdService.getUsersByRoleId(candidateGroup);
                    userList.addAll(usersByRoleId);
    ...
    public List<SysUser> getAssigneeSysUserFromTask(UserTask userTask) {
        String assignee = userTask.getAssignee();
        if (StrUtil.isNotBlank(assignee)) {
            // 指定单人
            SysUser userByUsername = iFlowThirdService.getUserByUsername(assignee);
    ...
    public List<SysUser> getCandidateSysUserFromTask(UserTask userTask) {
        List<String> candidateUsers = userTask.getCandidateUsers();
        if (CollUtil.isNotEmpty(candidateUsers)) {
            // 指定多人
            List<SysUser> list = iFlowThirdService.getAllUser();
    ...
    public List<SysUser> getRoleSysUserFromTask(UserTask userTask) {
        List<String> candidateGroups = userTask.getCandidateGroups();
        if (CollUtil.isNotEmpty(candidateGroups)) {
            //    指定多组
            List<SysUser> userList = Lists.newArrayList();
    ...
    public List<SysUser> getOrganizationSysUserFromTask(UserTask userTask) {
        List<String> candidateGroups = userTask.getCandidateGroups();
        if (CollUtil.isNotEmpty(candidateGroups)) {
            //    指定多组
            List<SysUser> userList = Lists.newArrayList();
            for (String candidateGroup : candidateGroups) {
                if (candidateGroup.startsWith("O*_")) {
                    List<SysUser> usersByRoleId = iFlowThirdService.getUsersByOrganizationId(candidateGroup);
                    userList.addAll(usersByRoleId);
                }
            }
     ...
     */
    @Override
    public Boolean getUserFlowNode(String taskId) {
        ...
        if (task != null && StringUtils.isNotBlank(task.getAssignee())) {
            ...
            try {
                SysUser loginUser = iFlowThirdService.getLoginUser();
                if (assign.contains(loginUser.getUsername())) {
                    result = true;
                }
            } catch (Exception e) {
                //忽略
     ...
     */
    @Override
    public Boolean isFirstUserTask(String taskId) {
        Boolean result = false;
        // 当前任务 task
        Task task = taskService.createTaskQuery().taskId(taskId).singleResult();
        // 获取流程定义信息
